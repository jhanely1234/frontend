...
export default function Reservas() {
  ...
  const fuse = useMemo(() => {
    return new Fuse(reservas, {
      keys: ['paciente.name', 'paciente.lastname', 'medico.name', 'medico.lastname', 'especialidad_solicitada.name'],
      threshold: 0.3,
      includeScore: true
    });
  }, [reservas]);
  ...
  const filteredReservas = useMemo(() => {
    ...
    if (filters.paciente || filters.medico || filters.especialidad) {
      const searchResults = fuse.search(filters.paciente || filters.medico || filters.especialidad);
      result = searchResults.map(result => result.item);
    }

    return Array.isArray(result) ? result.filter(reserva => 
      (!filters.fechaReserva || reserva.fechaReserva.split("T")[0] === filters.fechaReserva) &&
  ...
  const handleViewProfile = async (id) => {
    try {
      ...
      setError("Error al cargar los detalles de la reserva. Por favor, intente de nuevo.");
    }
  };
    ...
    try {
      ...
      setReservas(
        reservas.map((reserva) =>
          reserva._id === id ? { ...reserva, estado_reserva: nuevoEstado } : reserva
        )
      );
      return nuevoEstado;
    } catch (error) {
  ...
  const handleConfirmCancel = async (id) => {
    if (window.confirm("¿ESTÁ SEGURO QUE DESEA CANCELAR?")) {
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <button onClick={fetchReservas} className="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-300">
            Intentar de nuevo
          </button>
        </div>
      </div>
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-blue-800">Lista de Reservas</h1>
      ...
      {showFilters && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <input
            ...
            onChange={(e) => setFilters({...filters, paciente: e.target.value, medico: e.target.value})}
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="space-y-3">
              <p>
                ...
                {selectedReserva.paciente.name} {selectedReserva.paciente.lastname}
              </p>
              <p>
            ...
            {consultaDetalles && (
              <div className="space-y-3">
                ...
                <p>
                  ...
                  {new Date(consultaDetalles.citaMedica.fechaReserva).toLocaleDateString()}{" "}
